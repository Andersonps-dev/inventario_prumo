...
import { useBiparTransferencia } from '../../api/hooks';
import type { ListaProdutos } from '../../api/types';

/**
 * Painel de bipagem da transferência — origem já é fixa pra toda a
 * transferência (ao contrário do Grêmio), então não há etapa de "bipar
 * posição": só produto + quantidade, cada bipe soma (mesmo padrão do
 * ContagemRapida.tsx do inventário).
 */
export function TransferenciaBipagem({ transferenciaId }: { transferenciaId: number }) {
  const bipar = useBiparTransferencia();
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await bipar.mutateAsync({ id: transferenciaId, produtoId: produto.id, quantidade: quantidadeNum });
      setCodigo('');
      setQuantidade('');
  ...
  return (
    <div className="rounded-card border border-primary/30 bg-primary/5 p-4 shadow-sm md:shadow-none">
      <div className="mb-2 text-sm font-semibold text-ink">Bipagem da transferência</div>
